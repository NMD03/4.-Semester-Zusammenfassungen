...
languages_grouped_by_country AS (
	SELECT country_name,
	string_agg(DISTINCT language , ',') AS all_languages,
...
languages_per_region AS (
	...
	,continent_name
	FROM official_languages ol
	GROUP BY region_name, continent_name
)
...
languages_per_continent1 AS (
	SELECT continent_name,  string_agg(DISTINCT language , ',') AS languages,
...
region_grouped_by_language AS (
	SELECT language,
	string_agg(DISTINCT region_name , ',') , COUNT(DISTINCT region_name)
...
continents_grouped_by_language AS (
	SELECT language,
	string_agg(DISTINCT continent_name , ','), COUNT(DISTINCT continent_name),
	string_agg(DISTINCT region_name , ','), COUNT(DISTINCT region_name),
	string_agg(DISTINCT country_name , ','), COUNT(DISTINCT country_name)
	FROM official_languages ol
	GROUP BY language
)
