...
world_gdp_first AS 
(
	SELECT SUM(gdp) AS sum_gdp, YEAR FROM basic_data WHERE year IN (SELECT * FROM first_year)
)
...
world_gdp_last AS 
(
	SELECT SUM(gdp) AS sum_gdp, YEAR FROM basic_data WHERE year IN (SELECT * FROM last_year)
)
,
