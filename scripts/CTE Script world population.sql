...
(
	SELECT (SELECT sum_population FROM world_population_last)-(SELECT sum_population FROM world_population_first)
)
,
-- 2. Weltbevölkerung (Population) aus 1995 und 2015: relative Veränderung
world_population_diff_rate AS 
(
	SELECT (SELECT * FROM world_population_diff)/(SELECT sum_population FROM world_population_first)
)
